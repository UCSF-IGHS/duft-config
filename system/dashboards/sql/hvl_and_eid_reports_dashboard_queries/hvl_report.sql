...
hvl_backlog_before_reporting AS (
	...
	FROM (
		SELECT
			SUM(hvl_sample_received) - (SUM(hvl_sample_tested) + SUM(hvl_sample_rejected)) AS [Backlog at beginning of reporting period]
		FROM
...
hvl_backlog_after_reporting AS (
	...
	FROM (
		SELECT
			SUM(hvl_sample_received) - (SUM(hvl_sample_tested) + SUM(hvl_sample_rejected)) AS [Backlog at end of reporting period]
		FROM
			[final].fact_daily_sample_summary
		WHERE
