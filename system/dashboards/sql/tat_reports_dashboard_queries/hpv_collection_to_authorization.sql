...
(
    ...
    SELECT 2 AS sort_order,'15 to 21 days' AS tat_category, SUM(s.hpv_sample_collected_and_authorised_date_between_15_to_21_days)
    FROM final.fact_daily_sample_summary s
    WHERE s.report_date >= $[start_date%d]
      AND s.report_date <= $[end_date%d]
    ...
    SELECT 3 AS sort_order,'>21 days' AS tat_category, SUM(s.hpv_sample_collected_and_authorised_date_greater_than_21_days)
    FROM final.fact_daily_sample_summary s
    WHERE s.report_date >= $[start_date%d]
      AND s.report_date <= $[end_date%d]
) AS flattened
