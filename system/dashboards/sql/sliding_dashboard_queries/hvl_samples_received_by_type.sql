WITH DateRange AS (
SELECT
	DATEADD(DAY, -(DATEPART(WEEKDAY, GETDATE()) + 6), GETDATE()) AS firstDay,
	DATEADD(DAY, 1 - DATEPART(WEEKDAY, GETDATE()), GETDATE()) AS lastDay)
SELECT
	category,
	SUM(value) AS value
FROM
	...
	WHERE
		...
		AND s.report_date <= lastDay) AS SampleData UNPIVOT (value FOR category IN (Wholeblood, Plasma)) AS unpvt
GROUP BY
	category;