SELECT
    ...
    CAST(
        COALESCE(
            SUM(s.eid_sample_rejected) * 100.0 / NULLIF(SUM(s.eid_sample_dbs_received), 0),
            0
        ) AS DECIMAL(5, 2)
    ) AS value
FROM
...
INNER JOIN
    derived.dim_date d ON s.report_date = d.date
WHERE
    s.report_date >= DATEADD(DAY, -28, CAST(GETDATE() AS DATE))
